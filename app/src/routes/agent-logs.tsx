...
export default function AgentLogsPage() {
  const [agentOrder, setAgentOrder] = createSignal<string[]>([]);
  const [entriesById, setEntriesById] = createSignal<Record<string, AgentLogEntry>>({});
  ...
  const refreshLogs = async (): Promise<void> => {
    ...
    try {
      ...
      const nextOrder = logs.map((entry) => entry.agent.agent_id);
      ...
      syncScrollingAfterRefresh(nextOrder);
      setLastUpdated(new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" }));
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : "Failed to load agent logs");
    } finally {
      setLoading(false);
    }
  };
    ...
    <main class="flex h-screen flex-col bg-zinc-950 text-zinc-100">
      ...
      <div class="min-h-0 flex-1 overflow-y-auto px-6 py-6">
        <div class="mx-auto flex w-full max-w-7xl flex-col gap-6">
          ...
          {loading() ? <p class="text-zinc-400">Loading logs...</p> : null}
          {errorMessage() ? <p class="rounded-md bg-red-900/30 p-2 text-sm text-red-300">{errorMessage()}</p> : null}
